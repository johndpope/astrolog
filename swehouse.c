...
static int CalcH(double th, double fi, double ekl, char hsy, int iteration_count, struct houses *hsp);
static int sidereal_houses_ecl_t0(double tjde, double armc, double eps, double *nutlo, double lat, int hsys, double *cusp, double *ascmc);
static int sidereal_houses_trad(double tjde, double armc, double eps, double nutl, double lat, int hsys, double *cusp, double *ascmc);
static int sidereal_houses_ssypl(double tjde, double armc, double eps, double *nutlo, double lat, int hsys, double *cusp, double *ascmc);
 ...
 */
int FAR PASCAL_CONV swe_houses(double tjd_ut, double geolat, double geolon, int hsys, double *cusp, double *ascmc)
{
  int i, retc = 0;
 ...
 * ascmc[7] = polasc        * "polar ascendant" (M. Munkasey) *
 */
int FAR PASCAL_CONV swe_houses_ex(double tjd_ut, int32 iflag, double geolat, double geolon, int hsys, double *cusp, double *ascmc)
{
  int i, retc = 0;
...
static int sidereal_houses_ecl_t0(double tjde, double armc, double eps, double *nutlo, double lat, int hsys, double *cusp, double *ascmc)
{
  ...
  /* cartesian coordinates of an imaginary moving body on the the mean ecliptic of t0; we take the vernal point: */
  x[0] = x[4] = 1;
  x[1] = x[2] = x[3] = x[5] = 0;
  /* to equator */
  ...
  /* now, we have the moving point precessed to tjd_et. next, we compute the auxiliary epsilon: */
  ...
  /* distance of the auxiliary vernal point from the zero point at tjd_et (a section on the equator): */
  swi_cartpol(xvpx, x2);
  ...
  /* distance between auxiliary vernal point and vernal point of t0 (a section on the sidereal plane) */
...
static int sidereal_houses_ssypl(double tjde, double armc, double eps, double *nutlo, double lat, int hsys, double *cusp, double *ascmc)
{
  ...
  /* cartesian coordinates of the zero point on the the solar system rotation plane */
  x[0] = x[4] = 1;
  x[1] = x[2] = x[3] = x[5] = 0;
  /* to ecliptic 2000 */
  ...
  /* distance of the auxiliary vernal point from mean vernal point at tjd_et (a section on the equator): */
  swi_cartpol(xvpx, x2);
  dvpx = x2[0] * RADTODEG;    /* 2 */
  /* auxiliary armc */
  armcx = swe_degnorm(armc - dvpx);    /* 3 */
  /* compute axes and houses: */
  retc = swe_houses_armc(armcx, lat, epsx, hsys, cusp, ascmc);        /* 4 */
  /* distance between the auxiliary vernal point at t and the sidereal zero point of 2000 at t (a section on the sidereal plane). */
  dvpxe = acos(swi_dot_prod_unit(x, xvpx)) * RADTODEG;        /* 5 */
 ...
 * ascmc[7] = polasc        * "polar ascendant" (M. Munkasey) *
 */
int FAR PASCAL_CONV swe_houses_armc(double armc, double geolat, double eps, int hsys, double *cusp, double *ascmc)
{
  struct houses h;
  int i, retc = 0;
  int ito;
  ...
  /* for test of swe_house_pos(). 1st house will be 0, second 30, etc. */
  for(i = 1; i <= 12; i++) {
    double x[6];
 ...
 ***********************************************************/
{
  ...
  switch(hsy) {
    ...
    case 'H':
      ...
      /* within polar circle, when mc sinks below horizon and ascendant changes to western hemisphere, all cusps must be added 180 degrees. houses will
...
porphyry:
    ...
    case 'B': {            /* Alcabitius */
      /* created by Alois 17-sep-2000, followed example in Matrix electrical library. The code reproduces the example! See
         http://www.astro.com/cgi/adict.cgi query: alcabitius in the resuotl page, see program code example. I think the Alcabitius code in Walter
         Pullen's Astrolog 5.40 is wrong, because he remains in RA and forgets the transform to the ecliptic. */
      ...
      r = -tanfi * tand(dek);
      /* must treat the case of abs(r) > 1; probably does not happen because dek becomes smaller when fi is large, as ac is close to zero Aries/Libra
         in that case. */
      ...
      /* project rectasc onto eclipitic with pole height 0, i.e. along the declination circle */
    ...
    case 'U':              /* Krusinski-Pisa */
      ...
      for(i = 0; i < 6; i++) {
        /* B0. Set 'n-th' house cusp.  Note that IC/MC are also calculated here to check if really this is the asc-zenith great circle. */
        x[0] = 30.0 * i;
        x[1] = 0.0;
  ...
  /* with tropical latitudes, the vertex behaves strange, in a similar way as the ascendant within the polar circle. we keep it always on the western
     hemisphere. */
  if(fabs(fi) <= ekl) {
 ...
 */
double FAR PASCAL_CONV swe_house_pos(double armc, double geolat, double eps, int hsys, double *xpin, char *serr)
{
  ...
  switch(hsys) {
    ...
    case 'W':
      ...
      /* to make sure that a call with a house cusp position returns a value within the house, 0.001" is added */
    ...
    case 'B':              /* Alcabitius */
      ...
      /* while MC is always south, Asc must always be in eastern hemisphere */
      if(geolat >= 0 && 90 - geolat + demc < 0) {
        asc = swe_degnorm(asc + 180);
      }
    ...
    case 'K':
      ...
      break;
#endif
      /* version of Koch method: do calculations within circumpolar circle, if possible; make sure house positions 4 - 9 only appear on western
         hemisphere */
    case 'K':
      demc = atand(sind(armc) * tand(eps));
      is_invalid = FALSE;
    ...
    case 'C':
      ...
      break;
    case 'U':              /* Krusinski-Pisa */
      /* Purpose: find point where planet's house circle (meridian) cuts house plane, giving exact planet's house position. Input data: ramc, geolat,
         asc. */
      asc = Asc1(swe_degnorm(armc + 90), geolat, sine, cose);
      demc = atand(sind(armc) * tand(eps));
       ...
       * with this plane.
       */
      /* I. find plane of 'asc-zenith' great circle relative to equator: solve spherical triangle 'EP-asc-intersection of house circle with equator' */
      ...
      /* II. Next find asc and planet position on house plane, so to find relative distance of planet from coords beginning. */
      /* IIa. Asc on house plane relative to intersection of equator with 'asc-zenith' plane. */
      xasc[0] = asc;
      xasc[1] = 0.0;
      xasc[2] = 1.0;
      ...
      /* IIb. Planet on house plane relative to intersection of equator with 'asc-zenith' plane */
    ...
    case 'T':
      ...
      /* mirror everything below the horizon to the opposite point above the horizon */
      if(!is_above_hor) {

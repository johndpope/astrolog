...
int mon, day, yea, nFormat;
{
  ...
  char *current_locale;
  struct tm tm_time;
  char buffer[cchSzMax];
#endif

  if(us.fEuroDate) {
#ifdef PDF
    /* Get the name of the current locale.  */
    current_locale = strdup(setlocale(LC_ALL, NULL));

    /* Set new locale */
    setlocale(LC_ALL, gi.szLocale);
    /* Make time */
    tm_time.tm_year = yea - 1900;
...
#ifdef PDF
      case  9:
        strftime(buffer, cchSzMax, gi.szLocaleFormat, &tm_time);
        sprintf(szDat, "%s", buffer);
        break;
#endif
      case -1:

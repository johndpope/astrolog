...
static int read_elements_file(int32 ipl, double tjd,
                              ...
                              double *mano, double *sema, double *ecce, double *parg, double *node, double *incl, char *pname, int32 *fict_ifl, char *serr);
...
int swi_moshplan(double tjd, int ipli, AS_BOOL do_save, double *xpret, double *xeret, char *serr)
{
  ...
  if(tjd < MOSHPLEPH_START - 0.3 || tjd > MOSHPLEPH_END + 0.3) {
    if(serr != NULL) {
      sprintf(s, "jd %f outside Moshier planet range %.2f .. %.2f ", tjd, MOSHPLEPH_START, MOSHPLEPH_END);
  ...
  if(ipli == SEI_EARTH) {
    ...
    if(tjd == pdp->teval && pdp->iephe == SEFLG_MOSEPH) {
      ...
      /* one more position for speed. the following dt gives good speed for light-time correction */
#if 0
...
static void embofs_mosh(double tjd, double *xemb)
{
  ...
  L = L + 6.288750 * smp + 1.274018 * sx + 0.658309 * s2d + 0.213616 * s2mp - 0.185596 * sin(DEGTORAD * M)
      - 0.114336 * s2f;
 ...
 *   inclination
 */
#define SE_NEELY                /* use James Neely's revised elements * of Uranian planets */
...
#endif
  /* Isis-Transpluto; elements from "Die Sterne" 3/1952, p. 70ff. Strubell does not give an equinox. 1945 is taken to best reproduce ASTRON ephemeris. (This
     is a strange choice, though.) The epoch is 1772.76. The year is understood to have 366 days. The fraction is counted from 1 Jan. 1772 */
...
#if 0                           /* Ceres JPL 1600, without perturbations from other minor planets, * from following initial elements: * 2450600.5 2000 0 1
  164.7073602 73.0340746 80.5995101 * 10.5840296 0.07652422 0.0 2.770176095 */
  {
...
int swi_osc_el_plan(double tjd, double *xp, int ipl, int ipli, double *xearth, double *xsun, char *serr)
{
  ...
  /* orbital elements, either from file or, if file not found, from above built-in set */
  if(read_elements_file(ipl, tjd, &tjd0, &tequ, &mano, &sema, &ecce, &parg, &node, &incl, NULL, &fict_ifl, serr) == ERR)
    return ERR;
  ...
  while(fgets(s, AS_MAXCH, fp) != NULL) {
    ...
    if(mano != NULL) {
      ...
      *mano = swe_degnorm(*mano);
      ...
      /* if mean anomaly has t terms (which happens with fictitious planet Vulcan), we set epoch = tjd, so that no motion will be added anymore equinox =
         tjd */
      if(retc == 1) {
        *tjd0 = tjd;
      }

...
#if 0
#define DSUN    (1391978489.9 / AUNIT)  /* this value is consistent with * 959.63 arcsec at AU distance (Astr. Alm.) */
#else
...
static int32 eclipse_where(double tjd_ut, int32 ipl, char *starname, int32 ifl, double *geopos, double *dcore, char *serr);
static int32 eclipse_how(double tjd_ut, int32 ipl, char *starname, int32 ifl, double geolon, double geolat, double geohgt, double *attr, char *serr);
...
static int32 occult_when_loc(double tjd_start, int32 ipl, char *starname, int32 ifl, double *geopos, double *tret, double *attr, AS_BOOL backward, char *serr);
static int32 lun_eclipse_how(double tjd_ut, int32 ifl, double *attr, double *dcore, char *serr);
static int32 calc_mer_trans(double tjd_ut, int32 ipl, int32 epheflag, int32 rsmi, double *geopos, char *starname, double *tret, char *serr);
 ...
 */
int32 FAR PASCAL_CONV swe_sol_eclipse_where(double tjd_ut, int32 ifl, double *geopos, double *attr, char *serr)
{
...
int32 FAR PASCAL_CONV swe_lun_occult_where(double tjd_ut, int32 ipl, char *starname, int32 ifl, double *geopos, double *attr, char *serr)
{
  int32 retflag, retflag2;
  double dcore[10];
...
static int32 eclipse_where(double tjd_ut, int32 ipl, char *starname, int32 ifl, double *geopos, double *dcore, char *serr)
{
  ...
  /* nutation need not be in lunar and solar positions, if mean sidereal time will be used */
...
iter_where:
  ...
  /* Account for oblateness of earth: Instead of flattening the earth, we apply the correction to the z coordinate of the moon and the sun. This makes the
     calculation easier. */
  for(i = 0; i <= 2; i++)
    lx[i] = lm[i];
  ...
  s = s0 - d;
  /* next: geographic position of eclipse center. if shadow axis does not touch the earth, place on earth with maximum occultation is computed. */
  ...
  if(d == 0) {
    ...
    /* at edge of umbra and penumbra light rays are not parallel to shadow axis. for a short time close to contact of umbra and penumbra, an angle < 0.27
       degrees would have to be subtracted from b; */
  ...
  /* diameter of core shadow: first, distance moon - place of eclipse on earth */
  for(i = 0; i <= 2; i++)
    x[i] = rmt[i] - xst[i];
...
static int32 calc_planet_star(double tjd_et, int32 ipl, char *starname, int32 iflag, double *x, char *serr)
{
  ...
  if(starname == NULL || *starname == '\0') {
    ...
    if((retc = swe_fixstar(starname, tjd_et, iflag, x, serr)) == OK) {
      /* fixstars have the standard distance 1. in the occultation routines, this might lead to errors if interpreted as AU distance. To avoid this, we
         make it very high. */
      if(iflag & SEFLG_XYZ) {
        for(i = 0; i < 3; i++)
          x[i] *= 100000000;
 ...
 *
 */
int32 FAR PASCAL_CONV swe_sol_eclipse_how(double tjd_ut, int32 ifl, double *geopos, double *attr, char *serr)
{
  int32 retflag, retflag2;
  double dcore[10];
  double geopos2[20];
...
#endif
  attr[7] = dctr;
  /* approximate minimum height for visibility, considering refraction and dip 34.4556': refraction at horizon, from Bennets formulae 1.75' / sqrt(geohgt):
     dip of horizon 0.37' / sqrt(geohgt): refraction between horizon and observer */
 ...
 *
 */
int32 FAR PASCAL_CONV swe_sol_eclipse_when_glob(double tjd_start, int32 ifl, int32 ifltype, double *tret, int32 backward, char *serr)
{
...
next_try:
  ...
  /* approximate time of geocentric maximum eclipse formula from Meeus, German, p. 381 */
  tjd = 2451550.09765 + 29.530588853 * K + 0.0001337 * T2 - 0.000000150 * T3 + 0.00000000073 * T4;
  M = swe_degnorm(2.5534 + 29.10535669 * K - 0.0000218 * T2 - 0.00000011 * T3);
  Mm = swe_degnorm(201.5643 + 385.81693528 * K + 0.1017438 * T2 + 0.00001239 * T3 + 0.000000058 * T4);
  ...
  /* in extreme cases _where() returns no eclipse, where there is actually a very small one, therefore call _how() with the coordinates returned by _where():
   */
  ...
  if(retflag & SE_ECL_TOTAL) {
    ...
    /* the maximum is always total, and there is either one or to times before and after, when the core shadow becomes zero and totality changes into
       annularity or vice versa. */
 ...
 *
 */
int32 FAR PASCAL_CONV swe_lun_occult_when_glob(double tjd_start, int32 ipl, char *starname, int32 ifl, int32 ifltype, double *tret, int32 backward, char *serr)
{
  ...
  /* if (backward & SEI_OCC_FAST) dont_times = TRUE; */
  /* function calls for Pluto with asteroid number 134340 are treated as calls for Pluto as main body SE_PLUTO */
  if(ipl == SE_AST_OFFSET + 134340)
    ipl = SE_PLUTO;
...
next_try:
  ...
  for(i = 0; i < nstartpos; i++, t += direction * dadd2) {
    ...
    if(i > 1 && dc[i] > dc[i - 1] && dc[i - 2] > dc[i - 1]) {
      ...
      /* for (j = 0; j < nstartpos; j++) printf("%f ", dc[j]); */
      if(serr != NULL) {
        if(starname != NULL && *starname != '\0')
          strcpy(s, starname);
  ...
  /* in extreme cases _where() returns no eclipse, where there is actually a very small one, therefore call _how() with the coordinates returned by _where():
   */
  /* if ((retflag2 = eclipse_how(tjd, ipl, starname, ifl, geopos[0], geopos[1], 0, attr, serr)) == ERR) return retflag2; */
  if(retflag2 == 0) {
    /* only one try! */
    if(one_try) {
      tret[0] = tjd;
 ...
 *         declare as attr[20] at least !
 */
int32 FAR PASCAL_CONV swe_sol_eclipse_when_loc(double tjd_start, int32 ifl, double *geopos, double *tret, double *attr, int32 backward, char *serr)
{
  int32 retflag = 0, retflag2 = 0;
...
next_try:
  ...
  T3 = T2 * T;
  T4 = T3 * T;
  Ff = F = swe_degnorm(160.7108 + 390.67050274 * K - 0.0016341 * T2 - 0.00000227 * T3 + 0.000000011 * T4);
...
static int32 occult_when_loc(double tjd_start, int32 ipl, char *starname, int32 ifl, double *geopos, double *tret, double *attr, int32 backward, char *serr)
{
  int i, j, k, m;
  int32 retflag = 0;
...
next_try:
  ...
  for(i = 4; i >= 0; i--) {   /* attr for i = 0 must be kept !!! */
    ...
    if(eclipse_how(tret[i], ipl, starname, ifl, geopos[0], geopos[1], geopos[2], attr, serr) == ERR)
      return ERR;
 ...
 */
void FAR PASCAL_CONV swe_azalt(double tjd_ut, int32 calc_flag, double *geopos, double atpress, double attemp, double *xin, double *xaz)
{
  int i;
  double x[6], xra[3];
 ...
 */
void FAR PASCAL_CONV swe_azalt_rev(double tjd_ut, int32 calc_flag, double *geopos, double *xin, double *xout)
{
  int i;
  double x[6], xaz[3];
...
double FAR PASCAL_CONV swe_refrac(double inalt, double atpress, double attemp, int32 calc_flag)
{
  ...
  if(calc_flag == SE_TRUE_TO_APP) {
    ...
    /* Formula for low altitude is from the Almanac for Computers. It gives the correction for observed altitude, so has to be inverted numerically to get
       the observed from the true. Accuracy about 0.2' for -20C < T < +40C and 970mb < P < 1050mb. */
  ...
  if(calc_flag == SE_TRUE_TO_APP) {
    ...
    if(trualt > 15) {
      ...
      /* the following tan is not defined for a value of trualt near -5.00158 and 89.89158 */
    ...
    /* apparent to true */
    appalt = inalt;
    /* the following tan is not defined for a value of inalt near -4.3285 and 89.9225 */
...
{
#if 0
  /* formula based on G.G. Bennett, The calculation of astronomical refraction in marine navigation, * Journal of Inst. Navigation, No. 35, page 255-259,
     1982, * page 257 for refraction formula: formula H * and page 259 for atmospheric compensation */
  ...
  return r;
#else
  /* Formula by Sinclair, see article mentioned above, p. 256. Better for apparent altitudes < 0; */
  double r;
...
static double calc_dip(double geoalt, double atpress, double attemp, double lapse_rate)
{
  /* below formula is based on A. Thom, Megalithic lunar observations, 1973 (page 32). * conversion to metric has been done by * V. Reijs, 2000,
 ...
 *
 */
int32 FAR PASCAL_CONV swe_lun_eclipse_how(double tjd_ut, int32 ifl, double *geopos, double *attr, char *serr)
{
  double dcore[10];
 ...
 */
int32 FAR PASCAL_CONV swe_lun_eclipse_when(double tjd_start, int32 ifl, int32 ifltype, double *tret, int32 backward, char *serr)
{
...
next_try:
  ...
  M = swe_degnorm(2.5534 + 29.10535669 * kk - 0.0000218 * T2 - 0.00000011 * T3);
  Mm = swe_degnorm(201.5643 + 385.81693528 * kk + 0.1017438 * T2 + 0.00001239 * T3 + 0.000000058 * T4);
  Om = swe_degnorm(124.7746 - 1.56375580 * kk + 0.0020691 * T2 + 0.00000215 * T3);
  E = 1 - 0.002516 * T - 0.0000074 * T2;
...
int32 FAR PASCAL_CONV swe_pheno(double tjd, int32 ipl, int32 iflag, double *attr, char *serr)
{
  ...
  iflag = iflag & (SEFLG_EPHMASK | SEFLG_TRUEPOS | SEFLG_J2000 | SEFLG_NONUT | SEFLG_NOGDEFL | SEFLG_NOABERR | SEFLG_TOPOCTR);
  ...
  if(ipl != SE_SUN && ipl != SE_EARTH && ipl != SE_MEAN_NODE && ipl != SE_TRUE_NODE && ipl != SE_MEAN_APOG && ipl != SE_OSCU_APOG) {
    /*
  ...
  if(ipl > SE_AST_OFFSET || (ipl < NMAG_ELEM && mag_elem[ipl][0] < 99)) {
    if(ipl == SE_SUN) {
      ...
      u1 = atan2(sin(in) * tan(lbr2[1] * DEGTORAD)
                 + cos(in) * sin(lbr2[0] * DEGTORAD - om), cos(lbr2[0] * DEGTORAD - om)) * RADTODEG;
      u2 = atan2(sin(in) * tan(lbr[1] * DEGTORAD)
                 + cos(in) * sin(lbr[0] * DEGTORAD - om), cos(lbr[0] * DEGTORAD - om)) * RADTODEG;
      ...
      attr[4] = 5 * log10(lbr2[2] * lbr[2])
                + mag_elem[ipl][1] * sinB + mag_elem[ipl][2] * sinB * sinB + mag_elem[ipl][3] * du + mag_elem[ipl][0];

    } else if(ipl < SE_CHIRON) {
      attr[4] = 5 * log10(lbr2[2] * lbr[2])
                + mag_elem[ipl][1] * attr[0] / 100.0
                + mag_elem[ipl][2] * attr[0] * attr[0] / 10000.0 + mag_elem[ipl][3] * attr[0] * attr[0] * attr[0] / 1000000.0 + mag_elem[ipl][0];
...
#if 0
      {
        /* Expl. Suppl. to the Astronomical Almanac 1984, p. 400; Does not take into account - the topocentric distance of the moon - the distance of
           the observer from the geocenter */
...
#define SEFLG_EPHMASK   (SEFLG_JPLEPH|SEFLG_SWIEPH|SEFLG_MOSEPH)
int32 FAR PASCAL_CONV swe_rise_trans(double tjd_ut, int32 ipl, char *starname,
                                     int32 epheflag, int32 rsmi, double *geopos, double atpress, double attemp, double *tret, char *serr)
{
  ...
  *tret = 0;
  ...
  /* find culmination points within 28 hours from t0 - twohrs. culminations are required in case there are maxima or minima in height slightly above or below
     the horizon. we do not use meridian transits, because in polar regions the culmination points may considerably deviate from transits. also, there are
     cases where the moon rises in the western half of the sky for a short time. */
  if(do_fixstar) {
  ...
  /* note: there can be a rise or set on the poles, even if there is no culmination. So, we must not leave here in any case. */
...
int32 FAR PASCAL_CONV swe_nod_aps(double tjd_et, int32 ipl, int32 iflag, int32 method, double *xnasc, double *xndsc, double *xperi, double *xaphe, char *serr)
{
  int ij, i, j;
  int32 iplx;
  int32 ipli;
  ...
  if(ipl == SE_MEAN_NODE || ipl == SE_TRUE_NODE || ipl == SE_MEAN_APOG || ipl == SE_OSCU_APOG || ipl < 0 || (ipl >= SE_NPLANETS && ipl <= SE_AST_OFFSET)) {
  ...
  if((method == 0 || (method & SE_NODBIT_MEAN)) && ((ipl >= SE_SUN && ipl <= SE_NEPTUNE) || ipl == SE_EARTH)) {
    if(ipl == SE_MOON) {
    ...
    /* we need three positions and three speeds for three nodes/apsides. from the three node positions, the speed of the node will be computed. */
    if(ipli == SE_MOON) {
      dt = NODE_CALC_INTV;
      dzmin = 1e-15;
  ...
  /* to set the variables required in the save area, i.e. ecliptic, nutation, barycentric sun, earth we compute the planet */
  ...
  for(ij = 0, xp = xx; ij < 4; ij++, xp += 6) {
    ...
    if(do_aberr) {
      ...
      if(iflag & SEFLG_SPEED) {
        ...
        if(iflag & SEFLG_TOPOCTR) {
          /* geocentric position of observer */
          /* if (swi_get_observer(tjd_et - dt, iflag, FALSE, xobs, serr) != OK) return ERR; */
          for(i = 0; i <= 5; i++)
        ...
        /* The above call of swe_calc() has destroyed the parts of the save area (i.e. bary sun, earth nutation matrix!). to restore it: */
...
int32 FAR PASCAL_CONV swe_nod_aps_ut(double tjd_ut, int32 ipl, int32 iflag,
                                     int32 method, double *xnasc, double *xndsc, double *xperi, double *xaphe, char *serr)
{
  return swe_nod_aps(tjd_ut + swe_deltat(tjd_ut), ipl, iflag, method, xnasc, xndsc, xperi, xaphe, serr);
}
 ...
 */
int32 FAR PASCAL_CONV swe_gauquelin_sector(double t_ut, int32 ipl, char *starname, int32 iflag, int32 imeth, double *geopos, double atpress, double attemp,
    double *dgsect, char *serr)
{
  ...
  if(retval == ERR) {
    ...
    /* actually, we could return ERR here. However, we keep this variable, in case we implement an algorithm for Gauquelin sector positions of circumpolar
       bodies. As with the Ludwig Otto procedure with Placidus, one could replace missing rises or sets by meridian transits, although there are cases
       where even this is not possible. Sometimes a body both appears and disappears on the western part of the horizon. Using true culminations rather
       than meridan transits would not help in any case either, because there are cases where a body does not have a culmination within days, e.g. the sun
       near the poles. */
    rise_found = FALSE;
  }

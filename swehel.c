...
/* avoids problems with star name string that may be overwritten by
   swe_fixstar() */
int32 call_swe_rise_trans(double tjd, int32 ipl, char *star, int32 helflag, int32 eventtype, double *dgeo, double atpress, double attemp, double *tret,
                          char *serr)
{
  int32 retval;
...
static int32 calc_rise_and_set(double tjd_start, int32 ipl, double *dgeo, double *datm, int32 eventflag, int32 helflag, double *trise, char *serr)
{
  ...
  /* refraction for a body visible at the horizon at 0m above sea, atmospheric temperature 10 deg C, atmospheric pressure 1013.25 is 34.5 arcmin */
  rh = - (34.5 / 60.0 + rdi);
  ...
  /* now calculate more accurate rising and setting times. use vertical speed in order to determine crossing of the horizon refraction of 34' and solar disk
     diameter of 16' = 50' = 0.84 deg */
...
static double DistanceAngle(double LatA, double LongA, double LatB, double LongB)
{
  ...
  /* Haversine formula http://www.movable-type.co.uk/scripts/GIS-FAQ-5.1.html R.W. Sinnott, Virtues of the Haversine, Sky and Telescope, vol. 68, no. 2,
     1984, p. 159 */
...
static double kOZ(double AltS, double sunra, double Lat)
{
  ...
  /* depending on day/night vision (altitude of sun < start astronomical twilight), KO changes from 100% to 30% see extinction section of Vistas in Astronomy
     page 343 */
...
{
  /* depending on day/night vision (altitude of sun < start astronomical twilight), lambda eye sensibility changes see extinction section of Vistas in
     Astronomy page 343 */
  ...
  if(VR != 0) {
    if(VR >= 1) {
      /* Visbility range from http://www1.cs.columbia.edu/CAVE/publications/pdfs/Narasimhan_CVPR03.pdf
         http://www.icao.int/anb/SG/AMOSSG/meetings/amossg3/wp/SN11Rev.pdf where MOR=2.995/ke factor 1.3 is the relation between "prevailing visibility"
         and meteorological range was derived by Koshmeider in the 1920's */
      BetaVr = 3.912 / VR;
...
static double Deltam(double AltO, double AltS, double sunra, double Lat, double HeightEye, double *datm, int32 helflag, char *serr)
{
  ...
  if(staticAirmass == 0) {
    ...
    deltam =
      kR(AltS, HeightEye) * xR + kt(AltS, sunra, Lat, HeightEye, datm[1], datm[2], datm[3], 0, serr) * Xa + kOZ(AltS, sunra, Lat) * XOZ + kW(HeightEye,
          datm[1],
          datm[2]) *
      XW;
...
static double Bn(double AltO, double JDNDayUT, double AltS, double sunra, double Lat, double HeightEye, double *datm, int32 helflag, char *serr)
{
  ...
  /* YearB = DatefromJDut(JDNDayUT, 1); MonthB = DatefromJDut(JDNDayUT, 2); DayB = DatefromJDut(JDNDayUT, 3); */
...
' Pressure [mbar]
*/
static double Bm(double AltO, double AziO, double AltM, double AziM, double AltS, double AziS, double sunra, double Lat, double HeightEye, double *datm,
                 int32 helflag, char *serr)
{
  ...
  if(AltM > -0.26) {
    /* moon only adds light when (partly) above horizon From Schaefer , Archaeoastronomy, XV, 2000, page 129 */
...
' Pressure [mbar]
*/
static double Bsky(double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AltS, double AziS, double sunra, double Lat, double HeightEye,
                   double *datm, int32 helflag, char *serr)
{
  ...
  if(AltS < -3) {
    ...
    if(AltS > 4) {
      ...
      Bsky +=
        mymin(Bday(AltO, AziO, AltS, AziS, sunra, Lat, HeightEye, datm, helflag, serr),
...
static void default_heliacal_parameters(double *datm, double *dgeo, double *dobs, int helflag)
{
  ...
  if(datm[0] <= 0) {
    /* estimate atmospheric pressure, according to the International Standard Atmosphere (ISA) */
...
' VisLimMagn [-]
*/
static double VisLimMagn(double *dobs, double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AltS, double AziS, double sunra, double Lat,
                         double HeightEye, double *datm, int32 helflag, int32 *scotopic_flag, char *serr)
{
...
int32 FAR PASCAL_CONV swe_vis_limit_mag(double tjdut, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, double *dret, char *serr)
{
  ...
  if(strncmp(ObjectName, "moon", 4) == 0 || (helflag & SE_HELFLAG_VISLIM_DARK) || (helflag & SE_HELFLAG_VISLIM_NOMOON)
    ) {
    AltM = -90;
    AziM = 0;
  ...
  /* if (*serr != '\0') * in VisLimMagn(), serr is only a warning * retval = ERR; */
  return retval;
}
...
' TopoArcVisionis [deg]
*/
static int32 TopoArcVisionis(double Magn, double *dobs, double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AziS, double sunra,
                             double Lat, double HeightEye, double *datm, int32 helflag, double *dret, char *serr)
{
...
int32 FAR PASCAL_CONV swe_topo_arcus_visionis(double tjdut, double *dgeo, double *datm, double *dobs, int32 helflag, double mag, double azi_obj, double alt_obj,
    double azi_sun, double azi_moon, double alt_moon, double *dret, char *serr)
{
  double sunra = SunRA(tjdut, helflag, serr);
...
' HeliacalAngle [deg]
*/
static int32 HeliacalAngle(double Magn, double *dobs, double AziO, double AltM, double AziM, double JDNDaysUT, double AziS, double *dgeo, double *datm,
                           int32 helflag, double *dangret, char *serr)
{
...
int32 FAR PASCAL_CONV swe_heliacal_angle(double tjdut, double *dgeo, double *datm, double *dobs, int32 helflag, double mag, double azi_obj, double azi_sun,
...
static double WidthMoon(double AltO, double AziO, double AltS, double AziS, double parallax)
{
  ...
  return 0.27245 * parallax * (1 + sin(GeoAltO * DEGTORAD) * sin(parallax * DEGTORAD)) * (1 -
...
'29=MSk [-]
*/
int32 FAR PASCAL_CONV swe_heliacal_pheno_ut(double JDNDaysUT, double *dgeo, double *datm, double *dobs, char *ObjectNameIn, int32 TypeEvent, int32 helflag,
    double *darr, char *serr)
{
  ...
  /* note, the fixed stars functions rewrite the star name. The input string may be too short, so we have to make sure we have enough space */
    ...
    if(DeltaAlt < MinTAVact && Ta == 0 && Tc != 0) {
      ...
      Ta = TimePointer - TimeStep * (1 - crosspoint);
          ...
          !((TbVR != 0 && (TypeEvent == 3 || TypeEvent == 4)
             && (strncmp(ObjectName, "moon", 4) != 0 && strncmp(ObjectName, "venus", 5) != 0 && strncmp(ObjectName, "mercury", 7) != 0))));
...
#if 0
int32 FAR PASCAL_CONV HeliacalJDut(double JDNDaysUTStart, double Age, double SN, double Lat, double Longitude, double HeightEye, double Temperature,
...
{
  /* synodic periods from: Kelley/Milone/Aveni, "Exploring ancient Skies", p. 43. */
  switch(Planet) {
    case SE_MOON:
      return 29.530588853;
    case SE_MERCURY:
...
static int32 heliacal_ut_arc_vis(double JDNDaysUTStart, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 TypeEventIn, int32 helflag,
                                 double *dret, char *serr_ret)
{
  double x[6];
  double xin[2];
  double xaz[2];
  ...
  *serr = '\0';
  ...
  /* start values for search of heliacal rise maxlength = phase period in days, smaller than minimal synodic period */
...
static int32 get_asc_obl_with_sun(double tjd_start, int32 ipl, char *star, int32 helflag, int32 evtyp, double dperiod, double *dgeo, double *tjdret, char *serr)
{
  ...
  while(dsunpl_save == -999999999 ||
        ...
        fabs(dsunpl) + fabs(dsunpl_save) > 180 || (retro && !(dsunpl_save < 0 && dsunpl >= 0)) || (!retro && !(dsunpl_save >= 0 && dsunpl < 0))) {
    i++;
...
static int32 get_heliacal_day(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, int32 TypeEvent, double *thel, char *serr)
{
  ...
  for(tday = tjd; (direct_day > 0 && tday < tend) || (direct_day < 0 && tday > tend); tday += daystep * direct_day) {
    vdelta = -100;
    ...
    if(retval_old == -2 && retval >= 0 && daystep > 1) {
      ...
      /* Note: beyond latitude 55N (?), Mars can have a morning last. If the period of visibility is less than 5 days, we may miss the event. I don't
...
#if 0
static int32 get_acronychal_day_new(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, int32 TypeEvent, double *thel,
                                    char *serr)
{
  ...
  tret += 0.01 if((retval = my_rise_trans(tret, 0, ObjectName, SE_CALC_SET, helflag, dgeo, datm, &tret, serr)) == ERR)
            return ERR;
...
#if 0
static int32 get_acronychal_day_old(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, int32 TypeEvent, double *thel,
                                    char *serr)
{
  double tjdc = tjd, tret, x[6], xaz[6], AltO = -10;
  int32 retval, is_rise_or_set, iter_day;
...
static int32 time_optimum_visibility(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, double *tret, char *serr)
{
  ...
  *tret = tjd;
  ...
  for(i = 0, d = 100.0 / 86400.0; i < 3; i++, d /= 10.0) {
    while((retval = swe_vis_limit_mag(tjd - d, dgeo, datm, dobs, ObjectName, helflag, darr, serr)) >= 0 && darr[0] > darr[7]
          && darr[0] - darr[7] > vl) {
      tjd -= d;
      vl = darr[0] - darr[7];
      retval_sv = retval;
...
static int32 time_limit_invisible(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, int32 direct, double *tret, char *serr)
{
  ...
  *tret = tjd;
  ...
  for(i = 0, d = d0; i < ncnt; i++, d /= 10.0) {
    while((retval = swe_vis_limit_mag(tjd + d * direct, dgeo, datm, dobs, ObjectName, helflag, darr, serr)) >= 0 && darr[0] > darr[7]) {
      tjd += d * direct;
      retval_sv = retval;
      phot_scot_opic_sv = retval & SE_SCOTOPIC_FLAG;
      /* printf("%d: %f\n", direct, darr[8]); */
  ...
  *tret = tjd;
  /* if object disappears at setting, retval is -2, but we want it OK, and also suppress the warning "object is below local horizon" */
  *serr = '\0';
...
static int32 get_acronychal_day(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, int32 TypeEvent, double *thel,
                                char *serr)
{
...
static int32 get_heliacal_details(double tday, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 TypeEvent, int32 helflag, double *dret,
                                  char *serr)
{
  ...
  /* correct sequence of times: with event types 2 and 3 swap dret[0] and dret[2] */
  if(TypeEvent == 2 || TypeEvent == 3) {
    tday = dret[2];
    dret[2] = dret[0];
    dret[0] = tday;
...
static int32 heliacal_ut_vis_lim(double tjd_start, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 TypeEventIn, int32 helflag, double *dret,
                                 char *serr_ret)
{
  int i;
  ...
  *serr = '\0';
  ...
  if(ipl == SE_MERCURY)
    ...
    tjd = tjd_start - 50;   /* -50 makes sure, that no event is missed, * but may return an event before start date */
  ...
  if(!(helflag & SE_HELFLAG_NO_DETAILS)) {
    /* more precise event times for - morning first, evening last - venus and mercury's evening first and morning last */
    if(ipl == SE_MERCURY || ipl == SE_VENUS || TypeEvent <= 2) {
      ...
      for(i = 0, d = 100.0 / 86400.0; i < 3; i++, d /= 10.0) {
        while((retval = swe_vis_limit_mag(*dret + d * direct, dgeo, datm, dobs, ObjectName, helflag, darr, serr)) == -2
              || (retval >= 0 && darr[0] < darr[7])) {
          *dret += d * direct;
        }
      }
...
static int32 moon_event_vis_lim(double tjdstart, double *dgeo, double *datm, double *dobs, int32 TypeEvent, int32 helflag, double *dret, char *serr_ret)
{
  ...
  eventtype = TypeEvent + SE_BIT_DISC_CENTER;
  /* check Synodic/phase Period */
  tjd = tjdstart - 30;        /* -50 makes sure, that no event is missed, * but may return an event before start date */
  ...
  dret[1] = tjd;
  /* find moment of becoming visible */
  /* Note: The on the day of fist light the moon may become visible already during day. It also may appear during day, disappear again and then reappear
     after sunset */
  direct = 1;
...
static int32 heliacal_ut(double JDNDaysUTStart, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 TypeEventIn, int32 helflag, double *dret,
                         char *serr_ret)
{
  int32 avkind = helflag & SE_HELFLAG_AVKIND;
...
' see http://www.iol.ie/~geniet/eng/atmoastroextinction.htm
*/
int32 FAR PASCAL_CONV swe_heliacal_ut(double JDNDaysUTStart, double *dgeo, double *datm, double *dobs, char *ObjectNameIn, int32 TypeEvent, int32 helflag,
                                      double *dret, char *serr_ret)
{
  ...
  *serr = '\0';
  ...
  if(helflag & SE_HELFLAG_AVKIND) {
    ...
    /* acronychal rising and setting (cosmic setting) are ill-defined. We do not calculate them with the "visibility limit method" */
  ...
  for(itry = 0; tjd < tjdmax && retval == -2; itry++, tjd += tadd) {
    *serr = '\0';
    ...
    /* if resulting event date < start date for search (tjd0): retry starting from half a period later. The event must be found now, unless there is none,
       as is often the case with Mercury */
    while(retval != -2 && *dret < tjd0) {
      tjd += tadd;
      *serr = '\0';
      retval = heliacal_ut(tjd, dgeo, datm, dobs, ObjectName, TypeEvent, helflag, dret, serr);

...
static int state(double et, int32 *list, int do_bary, double *pv, double *pvsun, double *nut, char *serr);
static int interp(double FAR *buf, double t, double intv, int32 ncfin, int32 ncmin, int32 nain, int32 ifl, double *pv);
...
static int32 fsizer(char *serr)
{
  ...
  /* ttl = ephemeris title, e.g. "JPL Planetary Ephemeris DE404/LE404 Start Epoch: JED= 625296.5-3001 DEC 21 00:00:00 Final Epoch: JED= 2817168.5 3001 JAN 17
     00:00:00c */
  fread((void *) &ttl[0], 1, 252, js->jplfptr);
  /* cnam = names of constants */
  fread((void *) js->ch_cnam, 1, 6 * 400, js->jplfptr);
  /* ss[0] = start epoch of ephemeris ss[1] = end epoch ss[2] = segment size in days */
  ...
  /* plausibility test of these constants. Start and end date must be between -20000 and +20000, segment size >= 1 and <= 200 */
  ...
  /* ipt[i+0]: coefficients of planet i start at buf[ipt[i+0]-1] ipt[i+1]: number of coefficients (interpolation order - 1) ipt[i+2]: number of intervals in
     segment */
...
#if 0                           /* we prefer to compute ksize to be comaptible with new DE releases */
...
static int interp(double FAR *buf, double t, double intv, int32 ncfin, int32 ncmin, int32 nain, int32 ifl, double *pv)
{
  ...
  /*
     | get correct sub-interval number for this set of coefficients and then | get normalized chebyshev time within that subinterval. */
  if(t >= 0)
    dt1 = floor(t);
...
static int state(double et, int32 *list, int do_bary, double *pv, double *pvsun, double *nut, char *serr)
{
  ...
  if(js->jplfptr == NULL) {
    ...
    /* ttl = ephemeris title, e.g. "JPL Planetary Ephemeris DE404/LE404 Start Epoch: JED= 625296.5-3001 DEC 21 00:00:00 Final Epoch: JED= 2817168.5 3001
       JAN 17 00:00:00c */
    ...
    /* ipt[i+0]: coefficients of planet i start at buf[ipt[i+0]-1] ipt[i+1]: number of coefficients (interpolation order - 1) ipt[i+2]: number of intervals
       in segment */
    ...
    /* check if start and end dates in segments are the same as in file header */
  ...
  for(i = 0; i < 10; ++i) {
    if(list[i] > 0) {
      interp(&buf[(int) ipt[i * 3] - 1], t, intv, ipt[i * 3 + 1], 3L, ipt[i * 3 + 2], list[i], &pv[i * 6]);
  ...
  if(list[10] > 0 && ipt[34] > 0) {
    interp(&buf[(int) ipt[33] - 1], t, intv, ipt[34], 2L, ipt[35], list[10], nut);
  }

  /* get librations if requested (and if on file) */
  if(list[11] > 0 && ipt[37] > 0) {
    interp(&buf[(int) ipt[36] - 1], t, intv, ipt[37], 3L, ipt[38], list[1], &pv[60]);
  }
...
static int read_const_jpl(double *ss, char *serr)
{
...
#if DEBUG_DO_SHOW
  {
    ...
    for(i = 0; i < 13; i++) {
      ...
      printf("%s\t%d\tipt[%d]\t%3ld %2ld %2ld,\t", bname[i], i, j, js->eh_ipt[j], js->eh_ipt[j + 1], js->eh_ipt[j + 2]);
...
int swi_open_jpl_file(double *ss, char *fname, char *fpath, char *serr)
{
  ...
  if((js = (struct jpl_save *) CALLOC(1, sizeof(struct jpl_save))) == NULL
      || (js->jplfname = MALLOC(strlen(fname) + 1)) == NULL || (js->jplfpath = MALLOC(strlen(fpath) + 1)) == NULL) {
    if(serr != NULL)

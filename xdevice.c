...
void EndFileX()
{
  ...
  if(gs.fPS || !gs.fBitmap || gi.yOffset == 0) {
    fclose(gi.file);
    gi.yBand = 0;
  }
...
int PdfBegin()
{
  ...
  gi.hpdfDoc = HPDF_New(error_handler, NULL);
  HPDF_UseUTFEncodings(gi.hpdfDoc);
  HPDF_SetCurrentEncoder(gi.hpdfDoc, "UTF-8");
  ...
  if(setjmp(env)) {
    HPDF_Free(gi.hpdfDoc);
    return 1;
  }

  /* Add a new page object. */
  gi.hpdfPage = HPDF_AddPage(gi.hpdfDoc);
  HPDF_Page_SetSize(gi.hpdfPage, HPDF_PAGE_SIZE_A4, HPDF_PAGE_PORTRAIT);
  /*HPDF_Page_Concat(gi.hpdfPage, 72.0f / 300.0f, 0, 0, 72.0f / 300.0f, 0, 0); */ /* 300 dpi */
  HPDF_Page_SetTextLeading(gi.hpdfPage, 20);

//   HPDF_UseUTFEncodings(gi.hpdfDoc);


  gi.rPageHeight = HPDF_Page_GetHeight(gi.hpdfPage);
  gi.rPageWidth = HPDF_Page_GetWidth(gi.hpdfPage);

  /* Load ttf font */
  font_name = HPDF_LoadTTFontFromFile(gi.hpdfDoc, "/home/renner/.fonts/b/Benguiat.ttf", HPDF_TRUE);
  gi.hpdfTextFont = HPDF_GetFont(gi.hpdfDoc, font_name, "UTF-8");    //"StandardEncoding");

  /*   font_name = HPDF_LoadTTFontFromFile(gi.hpdfDoc, "/home/renner/.fonts/m/Morinus.ttf", HPDF_TRUE);
   *   gi.hpdfGlyphFont = HPDF_GetFont(gi.hpdfDoc, font_name, "ISO8859-15");
   */

  /* HPDF_SetCurrentEncoder(gi.hpdfDoc, "FontSpecific");
  /* gi.hpdfTextFont = HPDF_GetFont(gi.hpdfDoc, "Times-Roman", NULL); */
  /* Resolution of 72dpi: 72 pixels in one inch or 2.54cm.
   * 72 / 2.54 = 28.35 pixel / cm
   */
  /* printf("%f x %f\n", gi.rPageHeight, gi.rPageWidth);*/
...
void PdfWriteHeader()
{
  ...
  struct tm tm_time, *utc;
  time_t datetime;
  char *jpg_filename = LOGO;

  rect.left = margin;
  rect.top = gi.rPageHeight - margin;
  rect.right = margin + 0.8 * gi.rPageWidth;
  rect.bottom = rect.top - 2 * margin;

  HPDF_Page_Rectangle(gi.hpdfPage, rect.left, rect.bottom, rect.right - rect.left, rect.top - rect.bottom);
  HPDF_Page_Stroke(gi.hpdfPage);

  HPDF_Page_BeginText(gi.hpdfPage);

  /* Name */
//   gi.hpdfTextFont = HPDF_GetFont(gi.hpdfDoc, "Times-Bold", NULL);
//   gi.hpdfTextFont = HPDF_GetFont(gi.hpdfDoc, "Benguiat", "UTF-8");//"StandardEncoding");
//   hfont = HPDF_GetFont (pdf, font_name, "CP1250");
  /* gi.hpdfTextFont = HPDF_GetFont(gi.hpdfDoc, "Benguiat", NULL); */
  HPDF_Page_SetFontAndSize(gi.hpdfPage, gi.hpdfTextFont, 16);

  /* HPDF_Page_TextOut(gi.hpdfPage, rect.left, rect.top + 3, szPerson); */
#ifdef DEBUG
  PrintL();
  PrintDebug(szPerson);
#endif
  HPDF_Page_TextRect(gi.hpdfPage, rect.left, rect.top, rect.right, rect.bottom, szPerson, HPDF_TALIGN_LEFT, NULL);

  tm_time.tm_year = Yea - 1900;
  tm_time.tm_mon = Mon - 1;
  tm_time.tm_mday = Day;
  tm_time.tm_hour = HourFromTime(DecToDeg(Tim));
  tm_time.tm_min = MinFromTime(DecToDeg(Tim));
  tm_time.tm_sec = SecFromTime(DecToDeg(Tim));
  tm_time.tm_isdst = Dst; /* TODO: ??? */

  datetime = mktime(&tm_time);
  utc = gmtime(&datetime);
  ...
  HPDF_Page_SetFontAndSize(gi.hpdfPage, gi.hpdfTextFont, 8);

  /* Date */
  HPDF_Page_TextRect(gi.hpdfPage, rect.left, rect.top - 16, rect.right, rect.bottom, SzDate(Mon, Day, Yea, LOCALIZED), HPDF_TALIGN_LEFT, NULL);
#ifdef DEBUG
  PrintDebug(SzDate(Mon, Day, Yea, 9));
#endif


  /* Time */
  sprintf(sz, "%s (GMT %02d:%02d:%02d)", SzTime(tm_time.tm_hour, tm_time.tm_min, tm_time.tm_sec), (utc->tm_hour + UTC) % 24, utc->tm_min, utc->tm_sec);

  HPDF_Page_TextRect(gi.hpdfPage, rect.left, rect.top - (16 + LINE_HEIGHT), rect.right, rect.bottom, sz, HPDF_TALIGN_LEFT, NULL);

  /* Location */
  sprintf(sz, "%s %s", ciMain.loc, SzLocation(Lon, Lat));
#ifdef DEBUG
  PrintDebug(sz);
#endif
  HPDF_Page_TextRect(gi.hpdfPage, rect.left, rect.top - (16 + 2 * LINE_HEIGHT), rect.right, rect.bottom, sz, HPDF_TALIGN_LEFT, NULL);

  /* End text object. */
  HPDF_Page_EndText(gi.hpdfPage);
...
//   dst = HPDF_Page_CreateDestination(gi.hpdfPage);
//   HPDF_Destination_SetXYZ(dst, 0, gi.rPageHeight, 1);
//   HPDF_SetOpenAction(gi.hpdfDoc, dst);

  image = HPDF_LoadJpegImageFromFile(gi.hpdfDoc, jpg_filename);
  iw = HPDF_Image_GetWidth(image) / 2;
  ih = HPDF_Image_GetHeight(image) / 2;
#ifdef DEBUG
  sprintf(sz, "PageWidth x PageHeight: %f x %f", gi.rPageWidth, gi.rPageHeight);
  PrintDebug(sz);
  sprintf(sz, "ImageWidth x ImageHeight: %f x %f", iw, ih);
  PrintDebug(sz);
#endif
  HPDF_Page_DrawImage(gi.hpdfPage, image, rect.right, rect.top - ih, iw, ih);
  HPDF_Page_ExecuteXObject(gi.hpdfPage, image);
}


void PdfEnd()
{
  HPDF_SaveToFile(gi.hpdfDoc, gi.szFileOut);

  /* clean up */
  HPDF_Free(gi.hpdfDoc);
  PrintNotice("Creating Pdf chart file.");
}

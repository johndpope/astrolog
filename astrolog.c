...
#ifdef INTERPRET
    szMindPart[oSou] = "karmic past, and area of experience but little growth";
#endif
    ruler1[oSou] = sLeo;
  }
...
void InitVariables()
{
  ...
  ClearB((lpbyte) &us.fListing, (int)((lpbyte) &us.fLoop - (lpbyte) &us.fListing));
}
#endif
  ...
  switch(argv[0][pos]) {
    ...
    case 'Y':
      ...
      if(argc > 2) {
        ...
        if(!FBetween(i, nRightDirection, nLeftDirection)) {     /* left or right */
          if(ch1 == 'D')
            ErrorValN("YYD", i);
    ...
    switch(argv[0][ich - 1]) {
      ...
      case 'z':
        ...
        if(argc <= 1 || RParseSz(argv[1], pmZon) == rLarge) {
          if(argc > 1 && strcasecmp(argv[1], "current") == 0) {
            ...
            (void) time(&t);
            tm = localtime(&t);
            ZZ = us.zonDef = (tm->tm_isdst ? 1 : 0) - (real) tm->tm_gmtoff / 3600;
            argc--;
            argv++;
      ...
      case 'p':
        ...
        if(ch1 == 'd') {
          ...
          if(ch2 == 's') {
            SwitchF(fSolarArcKey); /* Solar arc in RA  */;
            break;
          }

          if(ch2 == 'b') {
            fBirthRate = 1 /* Birth rate in RA +- 1 day */;
            break;
          }

          if(ch2 == 'v') {
            fBirthRate = 2 /* Birth velocity in RA  */;
            break;
          }

          if(ch2 == 'n') {
            fBirthRate = 3 ;
            rBirthRate = rConverseBirthRate = 0.98564731 /* Naibod */;
            break;
          }

          if(ch2 == 'p') {
            us.rProgDay = 365.24218967 /* Ptolemy */;
            break;
          }

          if(ch2 == 'c') {
            fBirthRate = 4;
            rBirthRate = rConverseBirthRate = 0.98666666 /* Cardan */;
            break;
          }

 ...
 */
static double FAR z[] = {
  /* The following are scaled in arc seconds, time in Julian centuries. They replace the corresponding terms in the mean elements.  */
...
static short FAR LRT[8 * NLRT] = {
  /*
     Multiply by T Longitude Radius D l' l F .1" .00001" .1km .00001km */
...
int swi_moshmoon(double tjd, AS_BOOL do_save, double *xpmret, char *serr)
{
  ...
  if(tjd < MOSHLUEPH_START - 0.2 || tjd > MOSHLUEPH_END + 0.2) {
    if(serr != NULL) {
      sprintf(s, "jd %f outside Moshier's Moon range %.2f .. %.2f ", tjd, MOSHLUEPH_START, MOSHLUEPH_END);
  ...
  /* Moshier moon is referred to ecliptic of date. But we need equatorial positions for several reasons. e.g. computation of earth from emb and moon of
     heliocentric moon Besides, this helps to keep the program structure simpler */
  ecldat_equ2000(tjd, xpm);
  /* speed */
  /* from 2 other positions. */
  /* one would be good enough for computation of osculating node, but not for osculating apogee */
...
static void moon1()
{
  ...
  /* Note by Dieter: Bhanu noted that ss and cc are not sufficiently initialised and random values are used for the calculation. However, this may be only
     part of the bug. The bug could be in sscc(). Or may be the bug is rather in the 116th line of NLR, where the value "5" may be wrong. Still, this will
     make a maximum difference of only 0.1", while the error of the Moshier lunar ephemeris can reach 7". */
  int i, j;
...
int swi_mean_node(double J, double *pol, char *serr)
{
  ...
  if(J < MOSHNDEPH_START || J > MOSHNDEPH_END) {
    if(serr != NULL) {
      sprintf(s, "jd %f outside mean node range %.2f .. %.2f ", J, MOSHNDEPH_START, MOSHNDEPH_END);
  ...
  pol[0] = 125.0445550 - 1934.1361849 * T + 0.0020762 * T2 + T3 / 467410 - T4 / 60616000;
...
int swi_mean_apog(double J, double *pol, char *serr)
{
  ...
  if(J < MOSHNDEPH_START || J > MOSHNDEPH_END) {
    if(serr != NULL) {
      sprintf(s, "jd %f outside mean apogee range %.2f .. %.2f ", J, MOSHNDEPH_START, MOSHNDEPH_END);
  ...
  a = pol[0];
  /* Chapront, according to Meeus, German, p. 339 */
  pol[0] = 83.3532430 + 4069.0137111 * T - 0.0103238 * T2 - T3 / 80053 + T4 / 18999000;
...
#endif
  /* Lilith or Dark Moon is either the empty focal point of the mean lunar ellipse or, for some people, its apogee ("aphelion"). This is 180 degrees from the
     perigee. Since the lunar orbit is not in the ecliptic, the apogee must be projected onto the ecliptic. Joelle de Gravelaine has in her book "Lilith der
     schwarze Mond" (Astrodata, 1990) an ephemeris which gives noon (12.00) positions but does not project them onto the ecliptic. This results in a mistake
     of several arc minutes. There is also another problem. The other focal point doesn't coincide with the geocenter but with the barycenter of the
     earth-moon-system. The difference is about 4700 km. If one took this into account, it would result in an oscillation of the Black Moon. If defined as
     the apogee, this oscillation would be about +/- 40 arcmin. If defined as the second focus, the effect is very large: +/- 6 deg! We neglect this
     influence. */
...
void swi_mean_lunar_elements(double tjd, double *node, double *dnode, double *peri, double *dperi)
{
  T = (tjd - J2000) / 36525.0;
  T2 = T * T;
  mean_elements();
...
static void mean_elements()
{
  ...
  M += ((((((((1.62e-20 * T
               - 1.0390e-17) * T
              - 3.83508e-15) * T
             + 4.237343e-13) * T + 8.8555011e-11) * T - 4.77258489e-8) * T - 1.1297037031e-5) * T + 1.4732069041e-4) * T - 0.552891801772) * T2;
#ifdef MOSH_MOON_200
...
#endif                          /* ! MOSH_MOON_200 */
  /* sensitivity of mean elements delta argument = scale factor times delta amplitude (arcsec) cos l 9.0019 = mean eccentricity cos 2D 43.6 cos F 11.2
     (latitude term) */
}
...
void mean_elements_pl()
{
  ...
  Ve += ((((((((-9.36e-023 * T
                - 1.95e-20) * T
               + 6.097e-18) * T
              + 4.43201e-15) * T + 2.509418e-13) * T - 3.0622898e-10) * T - 2.26602516e-9) * T - 1.4244812531e-5) * T + 0.005871373088) * T2;
  Ea = mods3600(129597742.26669231 * T + 361679.214649);
  Ea += ((((((((-1.16e-22 * T
                + 2.976e-19) * T
               + 2.8460e-17) * T
              - 1.08402e-14) * T - 1.226182e-12) * T + 1.7228268e-10) * T + 1.515912254e-7) * T + 8.863982531e-6) * T - 2.0199859001e-2) * T2;

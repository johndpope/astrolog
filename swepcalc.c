...
static int plac2swe[] = {
  SE_SUN, SE_MOON, SE_MERCURY, SE_VENUS, SE_MARS, SE_JUPITER, SE_SATURN, SE_URANUS, SE_NEPTUNE, SE_PLUTO, SE_MEAN_NODE, SE_TRUE_NODE, SE_CHIRON,
  SE_MEAN_APOG, SE_CERES, SE_PALLAS, SE_JUNO, SE_VESTA,
};
             ...
             char *so)
{
   ...
   */
  sprintf(so, "%d,%.8f,%d,%d,%d,%d", id, jd_ad, flag, plalist, swe_d2l(ekl * DEG), swe_d2l(nut * DEG));
  so_len = strlen(so);
...
******************************************************************/
int calc(int planet,            /* planet index as defined in placalc.h, SUN = 0, MOON = 1 etc. planet == -1 calc calculates only nut, ekl, meanekl */
         double jd_ad,          /* relative Astrodienst Juldate, ephemeris time. Astrodienst Juldate is relative 31 Dec 1949, noon. */
     ...
     */
{
  ...
  /* earth: placalc makes no difference between sun and earth, swisseph does */
...
int fixstar(char *star, double jd, double *lon, double *lat)
{
  ...
  /* if call by number, fixstar() is 0-based, whereas swe_fixstar starts with 1 */
  if(isdigit((int) *star)) {
    i = atoi(star);
    sprintf(star, "%d", i + 1);
  }
...
/* ********************************************************* */
void CalcHouses(CSEC th, CSEC fi, CSEC mekl, char hsy, int iteration_count, struct houses *hsp)
{
  int retc = 0, i;
  ...
  retc = swe_houses_armc(th / cs2deg, fi / cs2deg, mekl / cs2deg, (int) hsy, cusps, ascmc);

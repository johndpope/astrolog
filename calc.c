...
real rDeg;
{
  ...
  if((hRevers < 2) || !PolarMCflip) {
    ...
    do {
      i++;
    } while(!(i >= cSign || (rDeg >= chouse[i] && rDeg < chouse[Mod12(i + 1)]) ||
              (chouse[i] > chouse[Mod12(i + 1)] && (rDeg >= chouse[i] || rDeg < chouse[Mod12(i + 1)]))));
...
void PrintRhythm(int i, int j, double s, int rhythm)
{
  int m, d, y;
  double t;
  char sz[cchSzDef];

  revjul(julday(MM, DD, YY, TT, 0) + s * us.rProgDay, 0, &m, &d, &y, &t);
  ...
  if(!FIgnore(i)) {
    if(!us.nMRRhythmY || us.nMRRhythmY == y) {
      ...
      sprintf(sz, "%3d year%s %3d day%s [%s]", (int) RFloor(s), (int) RFloor(s) == 1 ? " " : "s",
              (int)(RFract(s) * us.rProgDay), (int)(RFract(s) * us.rProgDay) == 1 ? " " : "s", SzDate(m, d, y, 3));
      PrintSz(sz);

      sprintf(sz, (rhythm == nLeftDirection ? (j == inhouse[i] ? "LD\n" : "L\n") : (j == inhouse[i] ? " DR\n" : "  R\n")));
      PrintSz(sz);
    }
  }
}
 ...
 * and positive if a > b. Multiplied result by 1000000
...
int cmpFactor(const void *a, const void *b)
{
  ...
  return (int)(1000000.f * ia->rFactor - 1000000.f * ib->rFactor);
}


void PrintAllRhythms()
{
  int i, j, dir;
  static int k = 0;
  double factor;
  ...
  struct _rhythm *rhythm = (struct _rhythm *) malloc(((us.nMRDir != 0 ? 2 : 1) * cObj * 12) * sizeof(struct _rhythm));
  rhythm_len = ((us.nMRDir == -1 ? 2 : 1) * cObj * 12) * sizeof(struct _rhythm) / sizeof(struct _rhythm);

  for(i = oSun; i <= cObj; i++) {
    /* if (us.nMRRhythmY || FIgnore(i) || FCusp(i)) */
    if(FIgnore(i) || FCusp(i))
      continue;

    if(!us.fSeconds && !us.nMRRhythmY) {
      PrintTab('-', 59);
      sprintf(sz, "\n");
      PrintSz(sz);
      AnsiColor(kObjA[i]);
      /* sprintf(sz, us.fSeconds ? "%-41s" : "%-44s", szObjName[i]); */
      sprintf(sz, "%-44s", szObjName[i]);
      PrintSz(sz);
    ...
    /* house position in time of rhythm */
    phouse[i] = (planet[i] - chouse[inhouse[i]] < 0 ? rDegMax + planet[i] - chouse[inhouse[i]] : planet[i] - chouse[inhouse[i]]) / shouse[inhouse[i]] * us.nMRRhythm;

    for(j = 1; j <= 12; j++) {
      /* left rhythm */
      if(us.nMRDir != nRightDirection) {
        factor = (j - 1) * us.nMRRhythm + phouse[i];
        dir = nLeftDirection;
        rhythm[k].nPlanet = i;
        rhythm[k].nHouse = j;
        rhythm[k].rFactor = factor;
        rhythm[k].nRhythm = dir;
        k++;

        if(!us.fSeconds)
          PrintRhythm(i, j, factor,  dir);
      }

      /* right rhythm */
      if(us.nMRDir != nLeftDirection) {
        factor = (13 - j) * us.nMRRhythm - phouse[i];
        dir = nRightDirection;
        rhythm[k].nPlanet = i;
        rhythm[k].nHouse = j;
        rhythm[k].rFactor = factor;
        ...
        if(!us.fSeconds)
          PrintRhythm(i, j, (12 - j) * us.nMRRhythm + us.nMRRhythm - phouse[i], nRightDirection);
      }
    }
  ...
  if(us.fSeconds) {
    ...
    for(l = 0; l < rhythm_len; l++)
      if(FBetween(rhythm[l].nPlanet, 0, cObj))
        if(FBetween(rhythm[l].nHouse, 1, 12))
          if(FBetween(rhythm[l].rFactor, 1e-10, rDegMax))
            PrintRhythm(rhythm[l].nPlanet, rhythm[l].nHouse, rhythm[l].rFactor, rhythm[l].nRhythm);
  }
...
bool FEnsureGrid()
{
  ...
  /*  if (cObj==cObjOpt) */
  grid = (GridInfo FPTR *) PAllocate(sizeof(GridInfo), fFalse, szType);
  /*  else

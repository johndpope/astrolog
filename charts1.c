...
void PrintHeader()
{
  ...
  if(us.fProgress || us.nRel == rcTransit || us.nRel == rcDual \
    ...
    if(us.fProgress && us.nRel) {
      ...
      sprintf(sz, "%c%c%c %s %s (%cT %s GMT) \n", chDay3(day),
              SzDate(Mon, Day, Yea, 3), SzTim(Tim), ChDst(Dst), SzZone(Zon));
      PrintSz(sz);
#endif
  ...
  if(us.fMunichRhythm) {
    sprintf(sz, "Triggers (%s %f years per house)\n",
        (us.nMRDir != 0 ? us.nMRDir == nRightDirection ? "Right Rhythm" : "Left Rhythm" : "Both Rhythms"), us.nMRRhythm);
    PrintSz(sz);
  }
...
void ChartListing()
{
  ...
  if(us.fSortRank) {
    ...
    offset = (us.nDegTo < 0) ? -us.nDegTo * 2 : (us.nDegTo > us.nSortDial) ?
            2 * (us.nSortDial - us.nDegTo) : 0 + (us.nDegFrom < 0) ? -2 * us.nDegFrom : (us.nDegFrom > us.nSortDial) ?
            2 * (us.nSortDial - us.nDegFrom) : 0;
  ...
  for(i = 1 - us.fSeconds, n = 0; i <= cObjOpt; i++, n++) {
    ...
    if(i <= cuspHi - 2  && n > cuspHi - 2)
      ...
      if(us.fSortRank) {
        ...
        /* sprintf(sz,"%f %f %f %f %f \n",value[j],f1,f2,us.nDegFrom,us.nDegTo);
         * PrintSz(sz);
         */
...
void ChartAstroGraph()
{
  ...
  if(us.fLatitudeCross) {
    if((c = (CrossInfo FPTR *)
            PAllocate(sizeof(CrossInfo), fFalse, szType)) == NULL)
      return;
  }

...
static const int pnoext2int[] = {
  SEI_SUN, SEI_MOON, SEI_MERCURY, SEI_VENUS, SEI_MARS, SEI_JUPITER, SEI_SATURN, SEI_URANUS, SEI_NEPTUNE, SEI_PLUTO, 0, 0, 0, 0, SEI_EARTH, SEI_CHIRON,
  SEI_PHOLUS, SEI_CERES, SEI_PALLAS, SEI_JUNO, SEI_VESTA,
};

static int do_fread(void *targ, int size, int count, int corrsize, FILE *fp, int32 fpos, int freord, int fendian, int ifno, char *serr);
static int get_new_segment(double tjd, int ipli, int ifno, char *serr);
static int main_planet(double tjd, int ipli, int32 epheflag, int32 iflag, char *serr);
static int main_planet_bary(double tjd, int ipli, int32 epheflag, int32 iflag, AS_BOOL do_save, double *xp, double *xe, double *xs, double *xm, char *serr);
static int32 swecalc(double tjd, int ipl, int32 iflag, double *x, char *serr);
static int sweplan(double tjd, int ipli, int ifno, int32 iflag, AS_BOOL do_save, double *xp, double *xpe, double *xps, double *xpm, char *serr);
static int swemoon(double tjd, int32 iflag, AS_BOOL do_save, double *xp, char *serr);
static int sweph(double tjd, int ipli, int ifno, int32 iflag, double *xsunb, AS_BOOL do_save, double *xp, char *serr);
static int jplplan(double tjd, int ipli, int32 iflag, AS_BOOL do_save, double *xp, double *xpe, double *xps, char *serr);
...
static int app_pos_rest(struct plan_data *pdp, int32 iflag, double *xx, double *x2000, struct epsilon *oe, char *serr);
 ...
 */
int32 FAR PASCAL_CONV swe_calc(double tjd, int ipl, int32 iflag, double *xx, char *serr)
{
  int i, j;
  int32 iflgcoor;
  ...
  /* function calls for Pluto with asteroid number 134340 are treated as calls for Pluto as main body SE_PLUTO. Reason: Our numerical integrator takes into
     account Pluto perturbation and therefore crashes with body 134340 Pluto. */
  if(ipl == SE_AST_OFFSET + 134340)
    ipl = SE_PLUTO;

  /* if ephemeris flag != ephemeris flag of last call, we clear the save area, to prevent swecalc() using previously computed data for current calculation.
     except with ipl = SE_ECL_NUT which is not dependent on ephemeris, and except if change is from ephemeris = 0 to ephemeris = SEFLG_DEFAULTEPH or
     vice-versa. */
  ...
  /* if (iflag & SEFLG_ICRS) iflag |= SEFLG_J2000; */
  ...
  if((iflag & SEFLG_SPEED3) == 0) {
    ...
    switch(ipl) {
      ...
      case SE_OSCU_APOG:
      case SE_TRUE_NODE:
        /* this is the optimum dt with Moshier ephemeris, but not with JPL ephemeris or SWISSEPH. To avoid completely false speed in case that JPL is
           wanted but the program returns Moshier, we use Moshier optimum. For precise speed, use JPL and FAST speed computation, */
...
int32 FAR PASCAL_CONV swe_calc_ut(double tjd_ut, int32 ipl, int32 iflag, double *xx, char *serr)
{
  ...
  if(ipl == SE_ECL_NUT) {
    ...
    switch(epheflag) {
      ...
      case SEFLG_JPLEPH:
        ...
        /* jpl ephemeris not on disk or date beyond ephemeris range * or file corrupt */
...
sweph_moon:
        ...
        /* for hel. or bary. position, we need earth and sun as well; * this is done by sweplan(), but not by swemoon() */
        if(iflag & (SEFLG_HELCTR | SEFLG_BARYCTR | SEFLG_NOABERR))
          retc = sweplan(tjd, ipli, SEI_FILE_MOON, iflag, DO_SAVE, NULL, NULL, NULL, NULL, serr);
...
moshier_moon:
    ...
    /* barycentric sun must be handled separately because of the following reasons: ordinary planetary computations use the function main_planet() and its
       subfunction jplplan(), see further below. now, these functions need the swisseph internal planetary indices, where SEI_EARTH = SEI_SUN = 0.
       therefore they don't know the difference between a barycentric sun and a barycentric earth and always return barycentric earth. to avoid this
       problem, many functions would have to be changed. as an alternative, we choose a more separate handling. */
    ...
    switch(epheflag) {
      ...
      case SEFLG_SWIEPH:
sweph_sbar:
        /* sweplan() provides barycentric sun as a by-product in save area; it is saved in swed.pldat[SEI_SUNBARY].x */
...
#else                           /* this code would be needed if barycentric moshier calculation * were implemented */
...
moshier_sbar:
    ...
    iflag = pedp->xflgs;
    /* barycentric sun is now in save area of barycentric earth. (pedp->xreturn = swed.pldat[SEI_EARTH].xreturn). in case a barycentric earth computation
       follows for the same date, the planetary functions will return the barycentric SUN unless we force a new computation of pedp->xreturn. this can be
       done by initializing the save of iflag. */
    pedp->xflgs = -1;
    /******************************************
     * mercury - pluto                        *
     ******************************************/
            ...
            || ipl == SE_MERCURY
            || ipl == SE_VENUS
            || ipl == SE_MARS || ipl == SE_JUPITER || ipl == SE_SATURN || ipl == SE_URANUS || ipl == SE_NEPTUNE || ipl == SE_PLUTO || ipl == SE_EARTH) {
    ...
    /* to avoid infinitesimal deviations from latitude = 0 that result from conversions */
    ...
    /* to avoid infinitesimal deviations from r-speed = 0 that result from conversions */
     ...
     ***********************************************/
            ...
            || ipl == SE_PALLAS || ipl == SE_JUNO || ipl == SE_VESTA || ipl > SE_AST_OFFSET) {
    /* internal planet number */
    if(ipl < SE_NPLANETS)
      ipli = pnoext2int[ipl];
    ...
    if(ipli == SEI_CHIRON && (tjd < CHIRON_START || tjd > CHIRON_END)) {
      if(serr != NULL)
        sprintf(serr, "Chiron's ephemeris is restricted to JD %8.1f - JD %8.1f", CHIRON_START, CHIRON_END);
    ...
    if(ipli == SEI_PHOLUS && tjd < PHOLUS_START) {
      if(serr != NULL)
        sprintf(serr, "Pholus's ephemeris is restricted to the time after JD %8.1f", PHOLUS_START);
...
do_asteroid:
    ...
    /* app_pos_etc_plan() might have failed, if t(light-time) is beyond ephemeris range. in this case redo with Moshier */
    ...
    || ipl == SE_KRONOS
    || ipl == SE_APOLLON
    || ipl == SE_ADMETOS || ipl == SE_VULKANUS || ipl == SE_POSEIDON || ipl == SE_ISIS || ipl == SE_NEPTUNE_LEVERRIER || ipl == SE_NEPTUNE_ADAMS)
#endif
    /* internal planet number */
    ipli = SEI_ANYBODY;
    pdp = &swed.pldat[ipli];
...
do_fict_plan:
    ...
    if(swi_osc_el_plan(tjd, pdp->x, ipl - SE_FICT_OFFSET, ipli, pedp->x, psdp->x, serr) != OK)
      ...
      if(retc == ERR)
        ...
        /* app_pos_etc_plan_osc() might have failed, if t(light-time) is beyond ephemeris range. in this case redo with Moshier */
        if(retc == NOT_AVAILABLE || retc == BEYOND_EPH_LIMITS) {
          if(epheflag != SEFLG_MOSEPH) {
              iflag = (iflag & ~SEFLG_EPHMASK) | SEFLG_MOSEPH;
              epheflag = SEFLG_MOSEPH;
...
void FAR PASCAL_CONV swe_close(void)
{
  ...
  /* free planets data space */ for(i = 0; i < SEI_NPLANETS; i++) {
...
void FAR PASCAL_CONV swe_set_ephe_path(char *path)
{
  ...
  if((sp = getenv("SE_EPHE_PATH")) != NULL && strlen(sp) != 0 && strlen(sp) <= AS_MAXCH - 1 - 13) {
    strcpy(s, sp);
...
static int swemoon(double tjd, int32 iflag, AS_BOOL do_save, double *xpret, char *serr)
{
  ...
  /* if planet has already been computed for this date, return if speed flag has been turned on, recompute planet */
 ...
 */
static int sweplan(double tjd, int ipli, int ifno, int32 iflag, AS_BOOL do_save, double *xpret, double *xperet, double *xpsret, double *xpmret, char *serr)
{
  int i, retc;
  ...
  /* xps (barycentric sun) may be necessary because some planets on sweph file are heliocentric, other ones are barycentric. without xps, the heliocentric
     ones cannot be returned barycentrically. */
  ...
  if(do_earth) {
    ...
    if(tjd == pebdp->teval && pebdp->iephe == SEFLG_SWIEPH && (!speedf2 || speedf1)) {
      ...
      /* speed is needed, if 1. true position is being computed before applying light-time etc.  this is the position saved in pdp->x.  in this case,
         speed is needed for light-time correction. 2. the speed flag has been specified. */
 ...
 * the positions will be written into the save area (swed.pldat[ipli].x)
 */
static int jplplan(double tjd, int ipli, int32 iflag, AS_BOOL do_save, double *xpret, double *xperet, double *xpsret, char *serr)
{
  int i, retc;
  ...
  /* we assume Teph ~= TDB ~= TT. The maximum error is < 0.002 sec, corresponding to an ephemeris error < 0.001 arcsec for the moon */
  /* double tjd_tdb, T; T = (tjd - 2451545.0)/36525.0; tjd_tdb = tjd + (0.001657 * sin(628.3076 * T + 6.2401) + 0.000022 * sin(575.3385 * T + 4.2970) +
...
static int sweph(double tjd, int ipli, int ifno, int32 iflag, double *xsunb, AS_BOOL do_save, double *xpret, char *serr)
{
   ...
   ******************************/ if(fdp->fptr != NULL) {
    /* if tjd is beyond file range, close old file. if new asteroid, close old file. */
    if(tjd < fdp->tfstart || tjd > fdp->tfend || (ipl == SEI_ANYBODY && ipli != pdp->ibdy)) {
      fclose(fdp->fptr);
      fdp->fptr = NULL;
...
again:
  ...
  /* if first ephemeris file (J-3000), it might start a mars period after -3000. if last ephemeris file (J3000), it might end a 4000-day-period before 3000. */
  if(tjd < fdp->tfstart || tjd > fdp->tfend) {
    if(serr != NULL) {
      if(tjd < fdp->tfstart)
        sprintf(s, "jd %f < Swiss Eph. lower limit %f;", tjd, fdp->tfstart);

      else
        sprintf(s, "jd %f > Swiss Eph. upper limit %f;", tjd, fdp->tfend);
  ...
  if(pdp->segp == NULL || tjd < pdp->tseg0 || tjd > pdp->tseg1) {
    ...
    /* rotate cheby coeffs back to equatorial system. if necessary, add reference orbit. */
  ...
  t = t * 2 - 1;
  /* speed is needed, if 1. true position is being computed before applying light-time etc.  this is the position saved in pdp->x.  in this case, speed is
     needed for light-time correction. 2. the speed flag has been specified. */
  ...
  /* if planet wanted is barycentric sun and must be computed from heliocentric earth and barycentric earth: the computation above gives heliocentric earth,
     therefore we have to compute barycentric earth and subtract heliocentric earth from it. this may be necessary with calls from sweplan() and from
     app_pos_etc_sun() (light-time). */
  if(ipl == SEI_SUNBARY && (pdp->iflg & SEI_FLG_EMBHEL)) {
    /* sweph() calls sweph() !!! for EMB. Attention: a new calculation must be forced in any case. Otherwise EARTH (instead of EMB) will possibly taken
       from save area. to force new computation, set pedp->teval = 0 and restore it after call of sweph(EMB). */
    tsv = pedp->teval;
    pedp->teval = 0;
  ...
  /* asteroids are heliocentric. if JPL or SWISSEPH, convert to barycentric */
...
static int app_pos_etc_plan(int ipli, int32 iflag, char *serr)
{
  ...
  } else if(ipli == SEI_CHIRON || ipli == SEI_PHOLUS || ipli == SEI_CERES || ipli == SEI_PALLAS || ipli == SEI_JUNO || ipli == SEI_VESTA) {
    ifno = SEI_FILE_MAIN_AST;
  ...
  /* if the same conversions have already been done for the same date, then return */
  ...
  if(!(iflag & SEFLG_TRUEPOS)) {
    ...
    switch(epheflag) {
      ...
      default:
        ...
        /* if speed flag is true, we call swi_moshplan() for new t. * this does not increase position precision, but speed precision, which becomes
           better than 0.01"/day. for precise speed, we need earth as well. */
        if(iflag & SEFLG_SPEED && !(iflag & (SEFLG_HELCTR | SEFLG_BARYCTR))) {
          if(ibody == IS_PLANET)
          ...
          /* only speed is taken from this computation, otherwise position calculations with and without speed would not agree. The difference would
             be about 0.01", which is far below the intrinsic error of the moshier ephemeris. */
          for(i = 3; i <= 5; i++)
            xx[i] = xxsv[i];
        }
  ...
  if(!(iflag & SEFLG_HELCTR) && !(iflag & SEFLG_BARYCTR)) {
    ...
    /* earth and planets are barycentric with jpl and swisseph, but asteroids are heliocentric. therefore, add baryctr. sun */
...
static int app_pos_rest(struct plan_data *pdp, int32 iflag, double *xx, double *x2000, struct epsilon *oe, char *serr)
{
  ...
  /* now we have ecliptic cartesian coordinates */ for(i = 0; i <= 5; i++)
...
void FAR PASCAL_CONV swe_set_sid_mode(int32 sid_mode, double t0, double ayan_t0)
{
  ...
  if(sid_mode == SE_SIDM_J2000 || sid_mode == SE_SIDM_J1900 || sid_mode == SE_SIDM_B1950) {
...
void swi_precess_speed(double *xx, double t, int direction)
{
  ...
  /* first correct rotation. this costs some sines and cosines, but neglect might involve an error > 1"/day */
...
void swi_nutate(double *xx, int32 iflag, AS_BOOL backward)
{
  ...
  for(i = 0; i <= 2; i++) {
    if(backward)
      x[i] = xx[0] * swed.nut.matrix[i][0] + xx[1] * swed.nut.matrix[i][1] + xx[2] * swed.nut.matrix[i][2];

    else
      x[i] = xx[0] * swed.nut.matrix[0][i] + xx[1] * swed.nut.matrix[1][i] + xx[2] * swed.nut.matrix[2][i];
  }
  ...
  if(iflag & SEFLG_SPEED) {
    ...
    for(i = 0; i <= 2; i++) {
      if(backward)
        x[i + 3] = xx[3] * swed.nut.matrix[i][0] + xx[4] * swed.nut.matrix[i][1] + xx[5] * swed.nut.matrix[i][2];

      else
        x[i + 3] = xx[3] * swed.nut.matrix[0][i] + xx[4] * swed.nut.matrix[1][i] + xx[5] * swed.nut.matrix[2][i];
    }

    /* then apparent motion due to change of nutation during day. this makes a difference of 0.01" */
    for(i = 0; i <= 2; i++) {
      if(backward)
        xv[i] = xx[0] * swed.nutv.matrix[i][0] + xx[1] * swed.nutv.matrix[i][1] + xx[2] * swed.nutv.matrix[i][2];

      else
        xv[i] = xx[0] * swed.nutv.matrix[0][i] + xx[1] * swed.nutv.matrix[1][i] + xx[2] * swed.nutv.matrix[2][i];
...
void swi_aberr_light(double *xx, double *xe, int32 iflag)
{
  ...
  if(iflag & SEFLG_SPEED) {
    /* correction of speed the influence of aberration on apparent velocity can reach 0.4"/day */
    for(i = 0; i <= 2; i++)
 ...
 */ void swi_deflect_light(double *xx, double dt, int32 iflag)
{
  ...
  ue = dot_prod(u, e);
  qe = dot_prod(q, e);
  /* When a planet approaches the center of the sun in superior conjunction, the formula for the deflection angle as given in Expl. Suppl. p. 136 cannot be
     used. The deflection seems to increase rapidly towards infinity. The reason is that the formula considers the sun as a point mass. AA recommends to set
     deflection = 0 in such a case. However, to get a continous motion, we modify the formula for a non-point-mass, taking into account the mass distribution
  ...
  if(iflag & SEFLG_SPEED) {
    /* correction of speed influence of light deflection on a planet's apparent speed: for an outer planet at the solar limb with |v(planet) - v(sun)| = 1
       degree, this makes a difference of 7"/day. if the planet is within the solar disc, the difference may increase to 30" or more. e.g. mercury at
       j2434871.45: distance from sun 45" 1. speed without deflection 2d10'10".4034 2. speed with deflection 2d10'42".8460 (-speed flag) 3. speed with
       deflection 2d10'43".4824 (< 3 positions/ -speed3 flag) 3. is not very precise. Smaller dt would give result closer to 2., but will probably never be
       as good as 2, unless int32 doubles are used. (try also j2434871.46!!) however, in such a case speed changes rapidly. before being passed by the sun,
       the planet accelerates, and after the sun has passed it slows down. some time later it regains 'normal' speed. to compute speed, we do the same
       calculation as above with slightly different u, e, q, and find out the difference in deflection. */
...
static int app_pos_etc_sun(int32 iflag, char *serr)
{
   ...
   * observer: geocenter or topocenter
       ...
       ************************************//* if topocentric position is wanted */ if(iflag & SEFLG_TOPOCTR) {
    if(swed.topd.teval != pedp->teval || swed.topd.teval == 0) {
      if(swi_get_observer(pedp->teval, iflag, DO_SAVE, xobs, serr) != OK)
        return ERR;
   ...
   *******************************/
  if(!(iflag & SEFLG_TRUEPOS)) {
    /* number of iterations - 1 the following if() does the following: with jpl and swiss ephemeris: with geocentric computation of sun: light-time
       correction of barycentric sun position.  with heliocentric or barycentric computation of earth: light-time correction of barycentric earth position.
       with moshier ephemeris (heliocentric!!!): with geocentric computation of sun: nothing! (aberration will be done later) with heliocentric or
       barycentric computation of earth: light-time correction of heliocentric earth position. */
    if(pedp->iephe == SEFLG_JPLEPH || pedp->iephe == SEFLG_SWIEPH || (iflag & SEFLG_HELCTR) || (iflag & SEFLG_BARYCTR)) {
      for(i = 0; i <= 5; i++) {
        xearth[i] = xobs[i];
      ...
      for(j = 0; j <= niter; j++) {
        ...
        /* new position */
        switch(pedp->iephe) {
            /* if geocentric sun, new sun at t' if heliocentric or barycentric earth, new earth at t' */
#ifndef NO_JPL
          case SEFLG_JPLEPH:
...
static int app_pos_etc_moon(int32 iflag, char *serr)
{
  ...
  /* the conversions will be done with xx[]. */ for(i = 0; i <= 5; i++) {
    xx[i] = pdp->x[i];
    xxm[i] = xx[i];
  }
...
static int read_const(int ifno, char *serr)
{
  ...
  *sp = '\0';
  ...
  if(ifno == SEI_FILE_ANY_AST) {
    ...
    /* MPC number and name; will be analyzed below: search "asteroid name" */
    while(*sp == ' ')
      sp++;
  ...
  /* is file bigendian or littlendian? test first byte of test integer, which is highest if bigendian */
  c = (char *) &testendian;
  ...
  fdp->npl = nplan;
  /* which ones? */
  retc = do_fread((void *) fdp->ipl, nbytes_ipl, (int) nplan, sizeof(int), fp, SEI_CURR_FPOS, freord, fendian, ifno, serr);
  ...
  if(ifno == SEI_FILE_ANY_AST) {
    ...
    /* name of asteroid is taken from orbital elements record read above */
    ...
    if(i == fdp->ipl[0] - SE_AST_OFFSET) {
      ...
      /* older elements record structure: the name is taken from old name field */
   ...
   *************************************/
  /* clight, aunit, helgravconst, ratme, sunradius these constants are currently not in use */
  ...
  if(!freord && size == corrsize) {
    ...
    for(i = 0; i < count; i++) {
      for(j = size - 1; j >= 0; j--) {
        ...
        if(size != corrsize)
          if((fendian == SEI_FILE_BIGENDIAN && !freord) || (fendian == SEI_FILE_LITENDIAN && freord))
            k += corrsize - size;
...
static void rot_back(int ipli)
{
  ...
  /* construct right handed orthonormal system with first axis along origin of longitudes and third axis along angular momentum this uses the standard
     formulas for equinoctal variables (see papers by broucke and by cefola).  */
 ...
 */ static int lunar_osc_elem(double tjd, int ipl, int32 iflag, char *serr)
{
  ...
  ndp = &swed.nddat[ipl];
  /* if elements have already been computed for this date, return if speed flag has been turned on, recompute */
  flg1 = iflag & ~SEFLG_EQUATORIAL & ~SEFLG_XYZ;
  ...
  /* the geocentric position vector and the speed vector of the moon make up the lunar orbital plane. the position vector of the node is along the
     intersection line of the orbital plane and the plane of the ecliptic. to calculate the osculating node, we need one lunar position with speed. to
     calculate the speed of the osculating node, we need three lunar positions and the speed of each of them. this is relatively cheap, if the jpl-moon or
     the swisseph moon is used. with the moshier moon this is much more expensive, because then we need 9 lunar positions for three speeds. but one position
     and speed can normally be taken from swed.pldat[moon], which corresponds to three moshier moon calculations. the same is also true for the osculating
  ...
  /* there may be a moon of wrong ephemeris in save area force new computation: */
  ...
  switch(epheflag) {
    ...
    case SEFLG_JPLEPH:
      ...
      for(i = istart; i <= 2; i++) {
        ...
        /* light-time-corrected moon for apparent node this makes a difference of several milliarcseconds with the node and 0.1" with the apogee. the
           simple formual 'x[j] -= dt * speed' should not be used here. the error would be greater than the advantage of computation speed. */
...
moshier_moon:
#endif
      /* with moshier moon, we need a greater speed_intv, because here the node and apogee oscillate wildly within small intervals */
      ...
      for(i = istart; i <= 2; i++) {
        ...
        /* light-time-corrected moon for apparent node. can be neglected with moshier */
        if((iflag & SEFLG_TRUEPOS) == 0 && retc >= OK) {
  ...
  /* now we have the correct direction of the node, the intersection of the lunar plane and the ecliptic plane. the distance is the distance of the point
     where the tangent of the lunar motion penetrates the ecliptic plane. this can be very large, e.g. j2415080.37372. below, a new distance will be derived
     from the osculating ellipse. */
  ...
  for(j = 0; j <= 1; j++) {
    ...
    /* sideral: we return NORMAL equatorial coordinates, there are no sidereal ones */
    if(iflag & SEFLG_SIDEREAL) {
      /* to J2000 */
    ...
    ndp->iephe = iflag & SEFLG_EPHMASK;
#ifdef SID_TNODE_FROM_ECL_T0
    /* node and apogee are already referred to t0; nothing has to be done */
#else

    if(iflag & SEFLG_SIDEREAL) {
      /* node and apogee are referred to t; the ecliptic position must be transformed to t0 */
      /* rigorous algorithm */
      if((swed.sidd.sid_mode & SE_SIDBIT_ECL_T0)
          || (swed.sidd.sid_mode & SE_SIDBIT_SSY_PLANE)) {
        for(i = 0; i <= 5; i++)
        ...
        swi_cartpol_sp(ndp->xreturn + 6, ndp->xreturn);
        swi_cartpol_sp(ndp->xreturn + 18, ndp->xreturn + 12);
        /* traditional algorithm; this is a bit clumsy, but allows us to keep the sidereal code together */
      ...
      /* node and apogee are referred to t; the ecliptic position must be transformed to J2000 */
      for(i = 0; i <= 5; i++)
        x[i] = ndp->xreturn[18 + i];
...
static int intp_apsides(double tjd, int ipl, int32 iflag, char *serr)
{
  ...
  nut = &swed.nut;
  ndp = &swed.nddat[ipl];
  /* if same calculation was done before, return if speed flag has been turned on, recompute */
  flg1 = iflag & ~SEFLG_EQUATORIAL & ~SEFLG_XYZ;
  flg2 = ndp->xflgs & ~SEFLG_EQUATORIAL & ~SEFLG_XYZ;
  speedf1 = ndp->xflgs & SEFLG_SPEED;
  speedf2 = iflag & SEFLG_SPEED;
  ...
  if(iflag & SEFLG_SIDEREAL) {
    /* apogee is referred to t; the ecliptic position must be transformed to t0 */
    /* rigorous algorithm */
    if((swed.sidd.sid_mode & SE_SIDBIT_ECL_T0)
        || (swed.sidd.sid_mode & SE_SIDBIT_SSY_PLANE)) {
      for(i = 0; i <= 5; i++)
...
int swi_plan_for_osc_elem(int32 iflag, double tjd, double *xx)
{
  ...
  /* For sidereal calculation we need node refered* to ecliptic of t0 of ayanamsa * ********************************************** */
  if(iflag & SEFLG_SIDEREAL) {
    tjd = sip->t0;
...
void swi_check_nutation(double tjd, int32 iflag)
{
  ...
  if(!(iflag & SEFLG_NONUT)
    ...
    if(iflag & SEFLG_SPEED) {
      /* once more for 'speed' of nutation, which is needed for planetary speeds */
...
**********************************************************/
int32 FAR PASCAL_CONV swe_fixstar(char *star, double tjd, int32 iflag, double *xx, char *serr)
{
  int i;
  int star_nr = 0;
...
found:
  ...
  /* FK5 to ICRS, if jpl ephemeris is referred to ICRS With data that are already ICRS, epoch = 0 */
  if(epoch != 0) {
  ...
  if(((iflag & SEFLG_NOGDEFL) == 0 || (iflag & SEFLG_NOABERR) == 0)
      && (iflag & SEFLG_HELCTR) == 0 && (iflag & SEFLG_BARYCTR) == 0 && (iflag & SEFLG_TRUEPOS) == 0)
#endif
   ...
   ************************************/
  for(i = 0; i <= 5; i++)
    x[i] *= 10000;          /* great distance, to allow algorithm used with planets */
...
int32 FAR PASCAL_CONV swe_fixstar_ut(char *star, double tjd_ut, int32 iflag, double *xx, char *serr)
{
...
char *FAR PASCAL_CONV swe_get_planet_name(int ipl, char *s)
{
  ...
  /* function calls for Pluto with asteroid number 134340 are treated as calls for Pluto as main body SE_PLUTO */
  if(ipl == SE_AST_OFFSET + 134340)
    ipl = SE_PLUTO;
  ...
  switch(ipl) {
    ...
    default:
      ...
      if(ipl > SE_AST_OFFSET) {
        ...
        /* If there is a provisional designation only in ephemeris file, we look for a name in seasnam.txt, which can be updated by the user. Some old
           ephemeris files return a '?' in the first position. There are still a couple of unnamed bodies that got their provisional designation before
           1925, when the current method of provisional designations was introduced. They have an 'A' as the first character, e.g. A924 RC. The file
           seasnam.txt may contain comments starting with '#'. There must be at least two columns: 1. asteroid catalog number 2. asteroid name The
...
static void trace_swe_calc(int swtch, double tjd, int ipl, int32 iflag, double *xx, char *serr)
{
  ...
  switch(swtch) {
    ...
    case 2:
      ...
      if(swi_fp_trace_out != NULL) {
        fprintf(swi_fp_trace_out, "swe_calc: %f\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t", tjd, ipl, iflag, xx[0], xx[1], xx[2], xx[3], xx[4], xx[5]);
...
static void trace_swe_fixstar(int swtch, char *star, double tjd, int32 iflag, double *xx, char *serr)
{
  ...
  switch(swtch) {
    ...
    case 2:
      ...
      if(swi_fp_trace_out != NULL) {
        fprintf(swi_fp_trace_out, "swe_fixstar: %s\t%f\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t", star, tjd, iflag, xx[0], xx[1], xx[2], xx[3], xx[4], xx[5]);
...
int swi_get_observer(double tjd, int32 iflag, AS_BOOL do_save, double *xobs, char *serr)
{
  ...
  /* geocentric position of observer depends on sidereal time, which depends on UT. compute UT from ET. this UT will be slightly different from the user's
     UT, but this difference is extremely small. */ delt = swe_deltat(tjd);
  ...
  /* mean or apparent sidereal time, depending on whether or not SEFLG_NONUT is set */
  sidt = swe_sidtime0(tjd_ut, eps, nut);
  sidt *= 15;                 /* in degrees */
  /* length of position and speed vectors; the height above sea level must be taken into account. with the moon, an altitude of 3000 m makes a difference of
     about 2 arc seconds. height is referred to the average sea level. however, the spheroid (geoid), which is defined by the average sea level (or rather by
     all points of same gravitational potential), is of irregular shape and cannot easily be taken into account. therefore, we refer height to the surface of
     the ellipsoid. the resulting error is below 500 m, i.e. 0.2 - 0.3 arc seconds with the moon. */
  cosfi = cos(swed.topd.geolat * DEGTORAD);
  sinfi = sin(swed.topd.geolat * DEGTORAD);
  cc = 1 / sqrt(cosfi * cosfi + (1 - f) * (1 - f) * sinfi * sinfi);
  ss = (1 - f) * (1 - f) * cc;
  /* neglect polar motion (displacement of a few meters), as long as we use the earth ellipsoid */
  /* ... */
  /* add sidereal time */
...
int FAR PASCAL_CONV swe_time_equ(double tjd, double *E, char *serr)
{
  ...
  L0 = 280.4664567 + swe_degnorm(tau * 360007.6982779)
       + tau2 * 0.03032028 + tau3 * 1 / 49931 - tau4 * 1 / 15299 - tau5 * 1 / 1988000;
  swi_nutation(tjd, nutlo);
